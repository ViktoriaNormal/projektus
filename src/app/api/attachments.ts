...
export function getCommentAttachments(commentId: string) {
  return apiRequest<AttachmentResponse[]>(`/tasks/comments/${commentId}/attachments`);
}
...
export async function uploadCommentAttachment(commentId: string, file: File): Promise<AttachmentResponse> {
  const token = localStorage.getItem('access_token');
  const formData = new FormData();
  formData.append('file', file);

  const response = await fetch(`/api/v1/tasks/comments/${commentId}/attachments`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: formData,
  });
