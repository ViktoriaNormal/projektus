...
export default function ProjectRolesSection({
  projectId, projectType, refs, roles, onReload,
  ...
  async function handleAddRole() {
    ...
    try {
      ...
      setNewRoleName(""); setNewRoleDescription("");
    ...
    <div className="bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden">
      <div className="p-6">
        <div className="flex items-center justify-between mb-1">
          ...
          {!showAddRoleForm && (
            <button onClick={() => setShowAddRoleForm(true)}
        ...
        <p className="text-sm text-slate-500 mb-5">
          Роли определяют права доступа участников к функциям проекта. Роли доступны для назначения участникам.
        </p>
          ...
          <div className="p-4 border-2 border-purple-300 rounded-lg bg-purple-50 mb-4">
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => { setShowAddRoleForm(false); setNewRoleName(""); setNewRoleDescription(""); }}
              ...
              <div key={role.id} className="border border-slate-200 rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {!isAdmin && (
                      <button onClick={(e) => { e.stopPropagation(); handleRemoveRole(role.id); }}
                        className="p-1.5 text-red-500 hover:bg-red-50 rounded transition-colors" title="Удалить роль"
                            ...
                            <div key={areaDef.area} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg gap-3">
                              ...
                              <div className="flex gap-1 shrink-0">
                                {allAccessLevels.map((level) => (
                                  <button
                                    key={level.key}
                                    onClick={() => !isAdmin && handleUpdatePermission(role.id, permArea, level.key as ProjectRolePermission["access"])}
                                    disabled={isAdmin}
                                    ...
                                    title={isAdmin ? "Права администратора нельзя изменить" : ((level as any).description || level.name)}
                                  >
